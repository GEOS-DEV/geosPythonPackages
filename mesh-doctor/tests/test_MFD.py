...
def __create_hex_grid( nx: int = 2, ny: int = 2, nz: int = 2 ) -> vtk.vtkUnstructuredGrid:
...
def __add_permeability( mesh: vtk.vtkUnstructuredGrid, permeability_value: float = 1.0 ) -> vtk.vtkUnstructuredGrid:
...
def test_mfd_indicators_hex_grid( tmp_path: pathlib.Path ) -> None:
    """Test the MFD indicators on a simple hexagonal grid with known properties."""
    ...
    for ip in [ "QTPFA", "BdLVM" ]:
        ...
        options = Options( vtkOutput=VtkOutput( output=str( out ), isDataModeBinary=True ),
                           ip=ip,
