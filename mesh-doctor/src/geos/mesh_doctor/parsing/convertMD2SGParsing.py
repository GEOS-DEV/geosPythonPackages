...
def fillSubparser( subparsers: _SubParsersAction[ Any ] ) -> None:
    ...
    p.add_argument( '--' + __SKIP_CLEAN,
                    action='store_true',
                    help="Skip the collocated node cleanup step." )
...
def displayResults( options: Options, result: Result ) -> None:
    ...
    setupLogger.results( f"  Skip clean collocated: {result.skipCleanCollocated}" )
    setupLogger.results( f"  Skip filter volume cells: {result.skipFilterVolumeCells}" )
