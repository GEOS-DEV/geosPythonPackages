...
class DeckPlotting(vuetify.VCard):
    def __init__(self, source=None, **kwargs):
        ...
        with self:
            ...
            html_viewX = matplotlib.Figure(
                figure=self.ctrl.permeability(**self.ctrl.figure_size()),
    ...
    def permeability(self, **kwargs):
        ...
        for f in self.source.plots():
            for t in f.table_function:
                if t.name == "waterRelativePermeabilityTable":
                    ...
                    self.water_y = np.loadtxt(
                        self.source.input_file.path + "/" + t.voxel_file
                    )
        ...
        if (
            self.water_x is not None
            and self.water_y is not None
            and self.gaz_x is not None
