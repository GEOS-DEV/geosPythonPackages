...
class DeckTree(object):
    ...
    def get_mesh(self) -> str:
        return normalize_path(
            self.input_file.path + "/" + self.input_file.problem.mesh[0].vtkmesh[0].file
        )
    ...
    def write_files(self):
        ...
        for filepath, content in files.items():
            includeName: str = self.input_file.xml_parser.file_to_relative_path[
                filepath
            ]
    ...
    def _convert_to_snake_case(self, content: str) -> str:
        ...
        """
        return "".join(
            ["_" + char.lower() if char.isupper() else char for char in content]
        ).lstrip("_")
    ...
    def _convert_proxy_path_into_proxy_names(self, proxy_path: str) -> list[str]:
        ...
        return [
            self._convert_to_snake_case(proxy) for proxy in splitted_path_without_root
        ]

    def _set_base_model_properties(
        self, model: dict, proxy_path: str, properties: dict
    ) -> None:
        """
        ...
        for model_inverted in models:
            ...
            current_base_model = getattr(current_base_model, "model_validate")(
                properties
            )
