...
class my_own_acc_flux_etor( OperatorsBase ):
    ...
    def evaluate( self, state, values ):
        ...
        for j in range( nph ):
            values_np[ self.FLUX_OP + j * self.ne:self.FLUX_OP + j * self.ne +
                       self.nc ] = self.x[ j ] * self.rho_m[ j ] * self.kr[ j ] / self.mu[ j ]
...
class my_own_comp_etor( my_own_acc_flux_etor ):
    ...
    def evaluate( self, state, values ):
        ...
        solid_mole = solid_volume * self.property.density_ev[ 'solid' ].evaluate(
            pressure ) / self.property.Mw[ 'Solid' ]
...
class my_own_rate_evaluator( operator_set_evaluator_iface ):
    ...
    def evaluate( self, state, values ):
        ...
        try:
            mu_g = CP.PropsSI( 'V', 'T', self.temperature, 'P|gas', bar2pa( pressure ),
                               'CarbonDioxide' ) * 1000  # Pa * s
        except ValueError:
