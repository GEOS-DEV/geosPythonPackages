...
def run_darts_model(domain: str, xml_name: str, darts_model=None):
    ...
    while time < solver.maxTime:
        # choose new timestep
        if domain == '1D':
            if time < 48: solver.setDt(4.0)
            elif time < 240: solver.setDt(8.64)
            elif time < 3600: solver.setDt(86.4)
            elif time < 6 * 8640: solver.setDt(240.0)
            elif time < 2 * 86400: solver.setDt(900.0)
            else: solver.setDt(3600.0)
        elif domain == '2D':
            if time < 24: solver.setDt(4.0)
            elif time < 120: solver.setDt(8.64)
            elif time < 300: solver.setDt(2 * 8.64)
            elif time < 1400: solver.setDt(60.0)
            elif time < 4 * 3600: solver.setDt(300.0)
            elif time < 9 * 3600: solver.setDt(200.0)
            else: solver.setDt(100.0)
        if rank == 0:
