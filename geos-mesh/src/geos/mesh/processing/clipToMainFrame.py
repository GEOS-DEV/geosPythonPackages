...
def transform_mesh( input: vtkUnstructuredGrid, logger: logging.Logger | None = None ) -> vtkUnstructuredGrid:
    ...
    translation, theta, axis = __recenter_and_rotate( dsa.numpy_support.vtk_to_numpy( input.GetPoints().GetData() ),
                                                      logger )
...
def __local_frame( pts: np.ndarray ) -> tuple[ np.ndarray, np.ndarray, np.ndarray ]:
    ...
    ori, _, _ = __origin_bounding_box( pts )
    u = pts[ 1 ]
    v = u
    ...
    # utils
def __origin_bounding_box( pts: np.ndarray ) -> tuple[ np.ndarray, np.ndarray, np.ndarray ]:
...
def __recenter_and_rotate( pts: np.ndarray, logger: logging.Logger ) -> tuple[ np.ndarray, float, np.ndarray ]:
    ...
    """
    # find bounding box
    org, vmin, vmax = __origin_bounding_box( pts )
