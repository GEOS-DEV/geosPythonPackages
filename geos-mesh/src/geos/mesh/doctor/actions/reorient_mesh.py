...
logger = getLogger( "reorient_mesh" )
...
def __select_and_flip_faces( mesh_points: vtkPoints, colors: Dict[ FrozenSet[ int ], int ],
    ...
    fs: Tuple[ FaceStream, FaceStream ] = ( face_stream.flip_faces( color_to_nodes[ 0 ] ),
...
def reorient_mesh( mesh, cell_indices: Iterator[ int ] ) -> vtkUnstructuredGrid:
    ...
    logger.info( "Reorienting the polyhedron cells to enforce normals directed outward." )
