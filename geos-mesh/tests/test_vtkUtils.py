...
@pytest.mark.parametrize( "onpoints, expected", [
    ( True, { 'GLOBAL_IDS_POINTS': 1 } ),
    ( False, { 'CELL_MARKERS': 1, 'PERM': 3, 'PORO': 1, 'FAULT': 1, 'GLOBAL_IDS_CELLS': 1 } )
])
def test_getAttributesFromDataSet( vtkDataSetTest: vtkDataSet,
                                  onpoints: bool, expected: dict[ str, int ]) -> None:
    attributes: dict[ str, int ] = vtkutils.getAttributesFromDataSet( object=vtkDataSetTest,
                                                                     onPoints=onpoints )
...
def test_isAttributeInObjectDataSet( vtkDataSetTest: vtkDataSet,
                                    attributeName: str,
                                    onpoints: bool, expected: bool ) -> None:
...
def test_getNumberOfComponentsDataSet( vtkDataSetTest: vtkDataSet,
    attributeName: str,
    onpoints: bool,
    expected: int, ) -> None:
...
def test_getComponentNamesDataSet( vtkdatasetWithComponentNames: vtkDataSet, attributeName: str, onpoints: bool,
    ...
    obtained : tuple[ str, ...] = vtkutils.getComponentNamesDataSet( vtkdatasetWithComponentNames, attributeName, onpoints)
