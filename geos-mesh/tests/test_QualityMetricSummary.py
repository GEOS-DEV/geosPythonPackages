...
cellMetricIndexes_all: tuple[int,...] = (
    ...
    (vtkMeshQuality.QualityMeasureTypes.SHAPE, vtkMeshQuality.QualityMeasureTypes.VOLUME, vtkMeshQuality.QualityMeasureTypes.JACOBIAN, 100),
    (vtkMeshQuality.QualityMeasureTypes.SHAPE, vtkMeshQuality.QualityMeasureTypes.VOLUME, vtkMeshQuality.QualityMeasureTypes.MEAN_ASPECT_FROBENIUS, 100),
    (vtkMeshQuality.QualityMeasureTypes.SHAPE, vtkMeshQuality.QualityMeasureTypes.VOLUME, vtkMeshQuality.QualityMeasureTypes.ODDY, 100),
...
def test_StatsType_compute( test_case: TestCase_StatsType ) -> None:
    ...
    assert abs(obs - test_case.expected) < 1e-4, f"Observed value is {round(obs, 4)} whereas expected value is {test_case.expected} for {test_case.statType.getString()}."
...
def __generate_test_data() -> Iterator[ TestCase ]:
    ...
    """
    rd.seed(10)
    for cellMetricIndexes, cellType in zip( cellMetricIndexes_all,
                                            cellType_all,
                                            strict=True ):
        values: tuple[float, float, float, float, int] = (
            rd.uniform(0.0, 5.0), rd.uniform(0.0, 5.0), rd.uniform(0.0, 5.0), rd.uniform(0.0, 5.0), rd.randint(1, 100000),
        )
...
def __generate_failed_test_data() -> Iterator[ TestCase ]:
    ...
    Yields:
        Iterator[ TestCase ]: iterator on test cases
    """
    for metricIndexes, cellType in zip( metricIndexesFail_all,
                                      cellType_all,
                                      strict=True ):
...
def test_QualityMetricSummary_init() -> None:
    ...
    assert (stats.getAllCellStats().shape[0] == 5) and stats.getAllCellStats().shape[1] == 122, "Stats shape is wrong."
...
def test_QualityMetricSummary_setterGetter( test_case: TestCase ) -> None:
    ...
    for cellMetricIndex in test_case.cellMetricIndexes:
        for statType, val in zip(test_case.statTypes, test_case.values, strict=True):
            ...
            assert subSet[cellMetricIndex][statType.getIndex()] == val, f"Stats at ({cellMetricIndex}, {test_case.cellType}, {statType}) from getCellStatsFromCellType is exepected to be equal to {val}."

            subSet2: pd.DataFrame = stats.getStatsFromMetric(cellMetricIndex)
            assert subSet2[test_case.cellType][statType.getIndex()] == val, f"Stats at ({cellMetricIndex}, {test_case.cellType}, {statType}) from getStatsFromMetric is exepected to be equal to {val}."

            subSet3: pd.Series = stats.getStatsFromMetricAndCellType(cellMetricIndex, test_case.cellType)
            assert subSet3[statType.getIndex()] == val, f"Stats at ({cellMetricIndex}, {test_case.cellType}, {statType}) from getStatsFromMetricAndCellType is exepected to be equal to {val}."

            valObs: float = stats.getCellStatValueFromStatMetricAndCellType(cellMetricIndex, test_case.cellType, statType)
    ...
    for cellMetricIndex in test_case.otherMetricIndexes:
        for statType, val in zip(test_case.statTypes, test_case.values, strict=True):
            ...
            assert subSet4[statType.getIndex()] == val, f"Stats at ({cellMetricIndex}, {statType}) from getStatsFromMetric is exepected to be equal to {val}."
