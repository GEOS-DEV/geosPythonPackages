...
filenames_all: tuple[ str, ...] = ( "triangulatedSurface.vtu", "tetraVolume.vtu", )
...
qualityMetrics_all: tuple[set[int],...] = (
    ...
    (int(vtkMeshQuality.QualityMeasureTypes.SCALED_JACOBIAN), int(vtkMeshQuality.QualityMeasureTypes.EQUIANGLE_SKEW), int(vtkMeshQuality.QualityMeasureTypes.SQUISH_INDEX)),
)
cellTypeCounts_all: tuple[tuple[int,...],...] = ((26324, 0, 0, 0, 0, 0, 26324, 0,), (0, 0, 368606, 0, 0, 0, 0, 368606,))
metricsSummary_all = (
    ((1.07, 0.11, 1.00, 1.94, 26324.0), (1.07, 0.11, 1.00, 1.94, 26324.0), (64.59, 6.73, 60.00, 110.67, 26324.0)),
...
def __generate_test_data() -> Iterator[ TestCase ]:
    ...
    """
    for filename, cellType, qualityMetrics, cellTypeCounts, metricsSummary in zip(
        filenames_all, cellTypes_all, qualityMetrics_all, cellTypeCounts_all, metricsSummary_all, strict=True
    ):
