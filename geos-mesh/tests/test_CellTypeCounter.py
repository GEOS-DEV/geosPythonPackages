...
from vtkmodules.vtkCommonDataModel import (
    ...
    vtkCellTypes,
    vtkCell,
    VTK_TRIANGLE, VTK_QUAD, VTK_TETRA, VTK_VERTEX, VTK_POLYHEDRON, VTK_POLYGON, VTK_PYRAMID, VTK_HEXAHEDRON, VTK_WEDGE,
)
...
filename_all: tuple[str,...] = ("triangle_cell.csv", "quad_cell.csv", "tetra_cell.csv", "pyramid_cell.csv", "hexa_cell.csv")
...
def __generate_test_data_multi_cell() -> Iterator[ TestCase ]:
    ...
    for cellType, filename, nbPtsCell in zip(cellType_all2, filename_all2, nbPtsCell_all2, strict=True):
        ptsCoords: npt.NDArray[np.float64] = np.loadtxt(os.path.join(data_root, filename), dtype=float, delimiter=',')
