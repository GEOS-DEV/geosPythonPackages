...
def convert(parsed_options) -> Options:
    ...
    def check_discretizations(x, nx, title):
        if len(x) != len(nx) + 1:
            raise ValueError(
                f"{title} information (\"{x}\" and \"{nx}\") does not have consistent size."
            )
    ...
    def parse_fields(s):
        ...
        if support not in ("CELLS", "POINTS"):
            raise ValueError(
                f"Support {support} for field \"{name}\" must be one of \"CELLS\" or \"POINTS\"."
            )
        try:
            dim = int(dim)
            assert dim > 0
...
def fill_subparser(subparsers) -> None:
    ...
    p.add_argument(
        ...
        default=(),
        help=
        "Create fields on CELLS or POINTS, with given dimension (typically 1 or 3)."
    )
