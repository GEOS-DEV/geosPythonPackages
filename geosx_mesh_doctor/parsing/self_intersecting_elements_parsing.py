...
def fill_subparser(subparsers) -> None:
    ...
    p.add_argument(
        ...
        help=
        f"[float]: The tolerance in the computation. Defaults to your machine precision {__TOLERANCE_DEFAULT}."
    )


def display_results(options: Options, result: Result):
    logging.error(
        f"You have {len(result.intersecting_faces_elements)} elements with self intersecting faces."
    )
