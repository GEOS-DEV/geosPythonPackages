...
def convert_to_fracture_policy(s: str) -> FracturePolicy:
    ...
    raise ValueError(
        f"Policy {s} is not valid. Please use one of \"{', '.join(map(str, __POLICIES))}\"."
    )
...
def fill_subparser(subparsers) -> None:
    ...
    p.add_argument(
        ...
        required=True,
        help=
        f"[string]: The criterion to define the surfaces that will be changed into fracture zones. "
...
def convert(parsed_options) -> Options:
    ...
    field_values = frozenset(
        map(int, parsed_options[__FIELD_VALUES].split(",")))
    vtk_output = vtk_output_parsing.convert(parsed_options)
    vtk_fracture_output = vtk_output_parsing.convert(parsed_options,
                                                     prefix=__FRACTURE_PREFIX)
