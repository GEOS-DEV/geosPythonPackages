...
def fill_subparser(subparsers) -> None:
    ...
    p.add_argument(
        ...
        help=
        f"[int]: Defaults chunk size for parallel processing to {__CHUNK_SIZE_DEFAULT}"
    )
    p.add_argument(
        '--' + __NUM_PROC,
        type=int,
        ...
        help=
        f"[int]: Number of threads used for parallel processing. Defaults to your CPU count {__NUM_PROC_DEFAULT}."
    )
...
def display_results(options: Options, result: Result):
    if result.unsupported_polyhedron_elements:
        ...
        logging.info(
            "All the polyhedra (if any) can be converted to supported elements."
        )
    if result.unsupported_std_elements_types:
        ...
        logging.info(
            "All the standard vtk element types (if any) are supported.")
