...
def getHistorySeries(database,
    ...
    if indices is not None:
        ...
        if isiterable(indices):
            oob_idxs = list(
                filter(lambda idx: not 0 <= idx < data_series.shape[1],
                       indices))
            if len(oob_idxs) > 0:
                print(
    ...
    if components is not None:
        ...
        if isiterable(components):
            oob_comps = list(
                filter(lambda comp: not 0 <= comp < data_series.shape[2],
                       components))
            if len(oob_comps) > 0:
                print(
    ...
    return [(time_series[:, 0], data_series[:, idx, comp], idx, comp)
            for idx in indices for comp in components]


def commandLinePlotGen():
    parser = argparse.ArgumentParser(
        description=
        "A script that parses geosx HDF5 time-history files and produces time-history plots using matplotlib"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        "Which time-history variable collected by GEOSX to generate a plot file for."
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=
        "An optional list of specific indices in the most-recently specified set."
    )
    ...
    if not os.path.isfile(args.filename):
        ...
        with h5w(args.filename, mode='r') as database:
            for setname in args.sets:
                ds = getHistorySeries(database, args.variable, setname,
                                      args.indices, args.components)
                if ds is None:
                    result = -1
                    break
                figname = args.variable + ("_" + setname
                                           if setname is not None else "")
                fig, ax = plt.subplots()
