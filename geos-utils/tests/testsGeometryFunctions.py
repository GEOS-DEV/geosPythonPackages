...
basisTo0: tuple[
    npt.NDArray[np.float64], npt.NDArray[np.float64], npt.NDArray[np.float64]
] = (np.array([2.0, 3.0, 0.0]), np.array([4.0, 5.0, 0.0]), np.array([0.0, 0.0, 1.0]))
basisTo1: tuple[
    npt.NDArray[np.float64], npt.NDArray[np.float64], npt.NDArray[np.float64]
] = (np.array([1.0, 1.0, 0.0]), np.array([0.0, 1.0, 1.0]), np.array([1.0, 0.0, 1.0]))
basisTo2: tuple[
    npt.NDArray[np.float64], npt.NDArray[np.float64], npt.NDArray[np.float64]
] = (np.array([0.0, 2.0, 0.0]), np.array([-2.0, 0.0, 0.0]), np.array([0.0, 0.0, 2.0]))
...
class TestsGeometryFunctions(unittest.TestCase):
    def test_getChangeOfBasisMatrixToCanonic(self: Self) -> None:
        ...
        obtained: npt.NDArray[np.float64] = fcts.getChangeOfBasisMatrix(
            basisTo0, basisCanon
        )
    ...
    def test_getChangeOfBasisMatrix(self: Self) -> None:
        ...
        expected: npt.NDArray[np.float64] = np.array(
            [[2.5, 4.5, -0.5], [0.5, 0.5, 0.5], [-0.5, -0.5, 0.5]]
        )
        self.assertTrue(
            np.array_equal(np.round(obtained, 2), np.round(expected, 2), equal_nan=True)
        )
    ...
    def test_computeCoordinatesInNewBasisIdentity(self: Self) -> None:
        ...
        changeOfBasisMatrix: npt.NDArray[np.float64] = fcts.getChangeOfBasisMatrix(
            basisCanon, basisCanon
        )
        obtained: npt.NDArray[np.float64] = fcts.computeCoordinatesInNewBasis(
            vec, changeOfBasisMatrix
        )
    ...
    def test_computePlaneFrom3Points(self: Self) -> None:
        ...
        obtained: tuple[float, float, float, float] = fcts.computePlaneFrom3Points(
            pt1, pt2, pt3
        )
    ...
    def test_getPointSideAgainstPlaneAssertion(self: Self) -> None:
        ...
        self.assertRaises(
            AssertionError, fcts.getPointSideAgainstPlane, planePt, planePt, planeNormal
        )
    ...
    def test_getCellSideAgainstPlaneRandom(self: Self) -> None:
        ...
        for cellPtsCoords in listCellPtsCoords:
            side: bool = fcts.getCellSideAgainstPlane(
                cellPtsCoords, planePt, planeNormal
            )
            obtained += [side]
        expected = (True, False)
        self.assertSequenceEqual(obtained, expected)
