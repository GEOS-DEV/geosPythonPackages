        ...
        for ci in range(cellCount):
            ...
            for faceIndex in range(cell.GetNumberOfFaces()):
                ...
                faceCentre = numpy.mean(
                    numpy.array(
                        [
                            gridPoints.GetPoint(face.GetPointId(fi))
                            for fi in range(facePointCount)
                        ]
                    ),
                    axis=0,
                )
                for i in range(facePointCount):
                    ...
                    newCell = self.localGrid.InsertNextCell(
                        vtk.VTK_TRIANGLE, 3, trianglePoints
                    )
                    triangle = self.localGrid.GetCell(newCell)
                    xp = [-1.0e30] * 3
                    res = triangle.IntersectWithLine(
                        x1, x2, 1.0e-4, self.t, xp, self.pcoords, self.subId
                    )
                    if res != 0:
    ...
    for wellName, wellData in wells.items():
        ...
        for c in wellData:
            ...
            for zi, dz in enumerate(zz):
                if (dz[0] < z0 + ztol and z0 - ztol < dz[1]) or (
                    dz[0] < z1 + ztol and z1 - ztol < dz[1]
                ):
                    found = True
