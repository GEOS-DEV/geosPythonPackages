...
def build_model(
    d: SimulationDeck, collection: vtk.vtkPartitionedDataSetCollection, attr: str
) -> int:
    """_summary_
...
def _read_boxes(
    d: SimulationDeck, collection: vtk.vtkPartitionedDataSetCollection
) -> int:
    ...
    for idx, box in enumerate(boxes):
        ...
        collection.GetMetaData(count).Set(
            vtk.vtkCompositeDataSet.NAME(), "Box" + str(idx)
        )
        ...
        assembly.SetAttribute(
            idbox, "number_of_partitions", collection.GetNumberOfPartitions(count)
        )
    ...
    if path.suffix in CLASS_READERS:
        ...
        for i in range(int(attr_min), int(attr_max + 1)):
            ...
            if extract.GetOutputDataObject(0).GetNumberOfCells() != 0:
                ...
                collection.GetMetaData(count).Set(
                    vtk.vtkCompositeDataSet.NAME(), "Surface" + str(i - 1)
                )
        ...
        for i in range(int(attr_min), int(attr_max + 1)):
            ...
            if extract.GetOutputDataObject(0).GetNumberOfCells() != 0:
                ...
                collection.GetMetaData(count).Set(
                    vtk.vtkCompositeDataSet.NAME(), "Region" + str(i - 1)
                )
        ...
        for i in range(mb.GetNumberOfBlocks()):
            if mb.HasMetaData(i):
                ...
                if unstructuredGrid and unstructuredGrid.GetNumberOfPoints():            
                    ...
                    collection.GetMetaData(count).Set(
                        vtk.vtkCompositeDataSet.NAME(), blockName
                    )
...
def _generate_grid(
    mesh: ElementTree.Element, collection: vtk.vtkPartitionedDataSetCollection
) -> int:
...
def _merge_xml_nodes(
    existingNode: ElementTree.Element, targetNode: ElementTree.Element, level: int
) -> None:
