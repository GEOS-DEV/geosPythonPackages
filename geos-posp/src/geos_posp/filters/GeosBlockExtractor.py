...
class GeosBlockExtractor( VTKPythonAlgorithmBase ):
    ...
    def RequestData(
        ...
        try:
            ...
            if self.m_extractFaults:
                ...
                assert (
                    faultMeshPort
                    < self.GetNumberOfOutputPorts() ), "Number of output ports is unconsistent with output objects."
    ...
    def UpdateOutputPorts( self: Self ) -> None:
        ...
        if nOutputPort == 2:
            outputObject: OutputObjectEnum = ( OutputObjectEnum.FAULTS
                                               if self.m_extractFaults else OutputObjectEnum.WELLS )
    ...
    def getOutputFaults( self: Self ) -> vtkMultiBlockDataSet:
        ...
        """
        assert ( self.m_extractFaults ), "Extract fault option was set to False, turn on first."
    ...
    def getOutputWells( self: Self ) -> vtkMultiBlockDataSet:
        ...
        """
        assert ( self.m_extractWells ), "Extract well option was set to False, turn on first."
    ...
    def doExtraction( self: Self ) -> int:
        ...
        try:
            ...
            if self.m_extractFaults and not self.extractRegion( GeosDomainNameEnum.FAULT_DOMAIN_NAME ):
                self.m_logger.error( "Fault extraction failed." )
            if self.m_extractWells and not self.extractRegion( GeosDomainNameEnum.WELL_DOMAIN_NAME ):
