...
class TestsFunctionsGeomechanicsCalculator(unittest.TestCase):
    def test_SpecificGravity(self: Self) -> None:
        ...
        obtained: npt.NDArray[np.float64] = fcts.specificGravity(
            density, specificDensity
        )
        expected: npt.NDArray[np.float64] = np.array([2.6, 3.1, 2.8, np.nan, 2.9])
        self.assertTrue(
            np.array_equal(np.round(obtained, 2), np.round(expected, 2), equal_nan=True)
        )
    ...
    def test_YoungModulus(self: Self) -> None:
        ...
        expected: npt.NDArray[np.float64] = np.array(
            [8.58, 62.07, np.nan, np.nan, 245.45]
        )
        self.assertTrue(
            np.array_equal(np.round(obtained, 2), np.round(expected, 2), equal_nan=True)
        )
    ...
    def test_OedometricModulus(self: Self) -> None:
        ...
        expected: npt.NDArray[np.float64] = np.array(
            [13.85, 65.52, np.nan, np.nan, 173.89]
        )
        self.assertTrue(
            np.array_equal(np.round(obtained, 2), np.round(expected, 2), equal_nan=True)
        )
    ...
    def test_TotalStress(self: Self) -> None:
        ...
        obtained: npt.NDArray[np.float64] = fcts.totalStress(
            effectiveStress, biotCoefficient, pressure
        )
        expected: npt.NDArray[np.float64] = np.array(
            [
    ...
    def test_StressRatio(self: Self) -> None:
        ...
        obtained: npt.NDArray[np.float64] = fcts.stressRatio(
    ...
    def test_LithostaticStress(self: Self) -> None:
        ...
        zCoords: npt.NDArray[np.float64] = np.array(
            [-2.0, -100.0, -500.0, np.nan, -2500.0]
        )
        gravity: float = PhysicalConstants.GRAVITY

        obtained: npt.NDArray[np.float64] = fcts.lithostaticStress(
            zCoords, density, gravity
        )
        expected: npt.NDArray[np.float64] = np.array(
            [51012.0, 3041100.0, 13734000.0, np.nan, 71122500.0]
        )
        self.assertTrue(
            np.array_equal(np.round(obtained, 2), np.round(expected, 2), equal_nan=True)
        )
    ...
    def test_ElasticStrainFromBulkShear(self: Self) -> None:
        ...
        obtained: npt.NDArray[np.float64] = fcts.elasticStrainFromBulkShear(
    ...
    def test_ElasticStrainFromYoungPoisson(self: Self) -> None:
        ...
        obtained: npt.NDArray[np.float64] = fcts.elasticStrainFromYoungPoisson(
            deltaEffectiveStress, young, poissonRatio
        )
    ...
    def test_ReservoirStressPathReal(self: Self) -> None:
        ...
        obtained: npt.NDArray[np.float64] = fcts.reservoirStressPathReal(
    ...
    def test_ReservoirStressPathOed(self: Self) -> None:
        ...
        obtained: npt.NDArray[np.float64] = fcts.reservoirStressPathOed(
    ...
    def test_criticalTotalStressRatio(self: Self) -> None:
        ...
        obtained: npt.NDArray[np.float64] = fcts.criticalTotalStressRatio(
    ...
    def test_totalStressRatioThreshold(self: Self) -> None:
        ...
        obtained: npt.NDArray[np.float64] = fcts.totalStressRatioThreshold(
    ...
    def test_CriticalPorePressure(self: Self) -> None:
        ...
        obtained: npt.NDArray[np.float64] = fcts.criticalPorePressure(
            effectiveStress2, rockCohesion, frictionAngle
        )
        expected: npt.NDArray[np.float64] = np.array(
            [-302.43, -333.72, -348.1, -383.01, -438.03]
        )
        self.assertTrue(
            np.array_equal(np.round(obtained, 2), np.round(expected, 2), equal_nan=True)
        )
    ...
    def test_CriticalPorePressureThreshold(self: Self) -> None:
        ...
        criticalPorePressure: npt.NDArray[np.float64] = np.array(
            [149.64, 174.64, 194.64, 219.64, 224.64]
        )

        obtained: npt.NDArray[np.float64] = fcts.criticalPorePressureThreshold(
    ...
    def test_compressibility(self: Self) -> None:
        ...
        obtained: npt.NDArray[np.float64] = 1000.0 * fcts.compressibility(
            poissonRatio, bulkModulus, biotCoefficient, porosity
        )
        expected: npt.NDArray[np.float64] = np.array(
            [110.438, 49.016, np.nan, np.nan, 18.991]
        )
        self.assertTrue(
            np.array_equal(np.round(obtained, 3), np.round(expected, 3), equal_nan=True)
        )
    ...
    def test_shearCapacityUtilization(self: Self) -> None:
        ...
        # calculation
        obtained: npt.NDArray[np.float64] = fcts.shearCapacityUtilization(
            traction, rockCohesion, frictionAngle
        )
        expected: npt.NDArray[np.float64] = np.array(
            [0.899, 0.923, 0.982, 1.004, 1.048]
        )

        self.assertSequenceEqual(
            np.round(obtained, 3).flatten().tolist(), expected.tolist()
        )
    ...
    def test_computeStressPrincipalComponents(self: Self) -> None:
        ...
        obtained: list[float] = [
            round(val, 3) for val in fcts.computeStressPrincipalComponents(stressTensor)
        ]
    ...
    def test_computeStressPrincipalComponentsFromStressVector(self: Self) -> None:
        ...
        obtained: list[float] = [
            round(val, 3)
            for val in fcts.computeStressPrincipalComponentsFromStressVector(
                stressVector
            )
        ]
        expected: tuple[float, float, float] = (1.748, 2.471, 5.281)
    ...
    def test_computeNormalShearStress(self: Self) -> None:
        ...
        obtained: list[float] = [
            round(val, 3)
            for val in fcts.computeNormalShearStress(stressTensor, directionVector)
        ]
