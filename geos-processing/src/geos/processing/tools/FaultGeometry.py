...
loggerTitle = "Fault Geometry"

class FaultGeometry:
    """Handles fault surface extraction and normal computation with optimizations."""

    def __init__( self: Self, mesh: vtkUnstructuredGrid, faultValues: list[ int ], faultAttribute: str,
                  volumeMesh: vtkUnstructuredGrid, outputDir: str = ".", logger: logging.Logger | None = None ) -> None:
    ...
    def initialize( self: Self,
                    ...
                    saveContributionCells:bool = True ) -> tuple[ vtkUnstructuredGrid, dict[ int, vtkUnstructuredGrid ] ]:
        ...
        Returns:
            tuple[vtkUnstructuredGrid, dict[int, vtkUnstructuredGrid]]: fault surface mesh, adjacency map
        """
    ...
    def _saveContributingCells( self: Self ) -> None:
        ...
        self.logger.info( f"   💾 All contributing cells saved: {filenameAll}"
        ...
        self.logger.info( f"      ({self.contributingCellsPlus.GetNumberOfCells()} cells, {self.contributingCellsPlus.GetNumberOfPoints} points)" )
        ...
        self.logger.info( f"      ({self.contributingCellsMinus.GetNumberOfCells()} cells, {self.contributingCellsMinus.GetNumberOfPoints} points)" )
    ...
    def _buildAdjacencyMappingFaceSharing( self: Self,
                                           processFaultsSeparately: bool = True ) -> dict[ int, vtkUnstructuredGrid ]:
    ...
    def _findFaceSharingCells( self: Self, faultSurface: vtkUnstructuredGrid ) -> dict[int, dict[str, int]]:
        ...
        Returns:
            dict[int, dict[str, int]] : Mapping from the best epsilon found.
        """
    ...
    def _testEpsilon( self: Self, faultSurface: vtkUnstructuredGrid, locator: vtkCellLocator, epsilon: float,
                      faultCenters: npt.NDArray[np.float64], faultNormals: npt.NDArray[ np.float64 ], volCenters: npt.NDArray[np.float64] ) -> tuple[ dict[int, dict[str, int]], dict[ str, Any]] :
        ...
        Returns:
            tuple[ dict[int, dict[str, int]], dict[ str, Any]]: Mapping of plus and minus cells, statistics
        """
        mapping = {}
        nFoundBoth = 0
        nFoundOne = 0
    ...
    def diagnoseNormals(
        ...
        badOrtho = (
            (np.abs(dotNormT1) > 1e-3)
            | (np.abs(dotNormT2) > 1e-3)
            | (np.abs(dotT1T2) > 1e-3)
        )
        ...
        if nBad > 0:
            ...
            updateAttribute( surface, np.maximum.reduce(
