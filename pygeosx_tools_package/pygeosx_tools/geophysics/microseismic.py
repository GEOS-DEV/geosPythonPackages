...
class JointSet():
    ...
    def build_joint_set( self,
        ...
        self.sigma_key = wrapper.get_matching_wrapper_path(
            problem, [ self.mesh_level, self.region_name, self.solid_name, 'stress' ] )
        ...
        ################################################
        if self.fluid_name:
            self.pressure_key = wrapper.get_matching_wrapper_path( problem,
                                                                   [ self.mesh_level, self.region_name, 'pressure' ] )
    ...
    def choose_random_locations( self, problem, density ):
        ...
        node_position_key = wrapper.get_matching_wrapper_path( problem,
                                                               [ self.mesh_level, 'nodeManager', 'ReferencePosition' ] )
        node_element_key = wrapper.get_matching_wrapper_path( problem, [ self.mesh_level, 'nodeManager', 'elemList' ] )
        element_node_key = wrapper.get_matching_wrapper_path(
            problem, [ self.mesh_level, 'ElementRegions', self.region_name, 'nodeList' ] )
        ...
        for ii in range( Ntest ):
            ...
            R2 = ( x[ :, 0 ] - test_locations[ ii, 0 ] )**2 + ( x[ :, 1 ] - test_locations[ ii, 1 ] )**2 + (
                x[ :, 2 ] - test_locations[ ii, 2 ] )**2
            Ia = np.argmin( R2 )
        ...
        for ii in range( self.N ):
            if ( ( self.fail_count[ ii ] == 0 ) | self.allow_repeating ):
                ...
                if ( fail_criteria > 0 ):
                    ...
                    self.sigma_sdn_offset[ ii, 0, 2 ] += dT * np.cos( rake )
                    self.sigma_sdn_offset[ ii, 1, 2 ] += dT * np.sin( rake )
                    events.append( [ self.location[ ii, ...], self.strike[ ii ], self.dip[ ii ], rake ] )
        return events
    ...
    def choose_random_uniform_orientations( self ):
        ...
        """
        self.strike = np.random.uniform( low=0.0, high=2.0 * np.pi, size=self.N )
        self.dip = np.arccos( np.random.uniform( low=-1.0, high=1.0, size=self.N ) )
...
class MicroseismicAnalysis():
    ...
    def __init__( self ):
        ...
        self.joint_sets = []
        self.set_names = []
        self.catalog_order = [ 't', 'x', 'y', 'z', 'strike', 'dip', 'rake', 'set_id' ]
    ...
    def save_catalog_csv( self, fname ):
        ...
        if ( parallel_io.rank == 0 ):
            ...
            np.savetxt( '%s.csv' % ( fname ), catalog_data, fmt='%1.4f', header=catalog_header, delimiter=', ' )
    ...
    def save_catalog_vtk( self, fname ):
        ...
        if ( parallel_io.rank == 0 ):
            if len( self.catalog_all[ 'x' ] ):
                ...
                pointsToVTK( fname, np.zeros( 1 ), np.zeros( 1 ) - 1e-6, np.zeros( 1 ) + 1e-6, data=empty_catalog )
    ...
    def save_stereonet( self, fname, density=True, poles=True ):
        ...
        if len( self ):
            ...
            if density:
                cax = ax.density_contourf( self.catalog_all[ 'strike' ],
                                           self.catalog_all[ 'dip' ],
                                           measurement='poles' )
                f.colorbar( cax )
            if poles:
    ...
    def save_all_joints( self, fname ):
        ...
        """
        targets = [
            'strike', 'dip', 'mu', 'cohesion', 'location', 'fail_count', 'sigma_sdn_offset', 'sigma_n', 'pressure',
            'tau', 'sigma_origin', 'sigma_new'
        ]
