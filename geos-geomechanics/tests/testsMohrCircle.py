...
class TestsMohrCircle(unittest.TestCase):
    ...
    def test_MohrCircleInit(self: Self) -> None:
        ...
        obtained: list[float] = [
            round(val, 3) for val in mohrCircle.getPrincipalComponents()
        ]
    ...
    def test_MohrCircleRadius(self: Self) -> None:
        ...
        expected: float = (
            principalComponentsExpected[2] - principalComponentsExpected[0]
        ) / 2.0
...
class TestsMohrCoulomb(unittest.TestCase):
    ...
    def test_computeShearStress(self: Self) -> None:
        ...
        # instantiate object
        mohrCoulomb: MohrCoulomb = MohrCoulomb(rockCohesion, frictionAngle)
        obtained: npt.NDArray[np.float64] = np.array(
            mohrCoulomb.computeShearStress(stressNormal)
        )
    ...
    def test_computeFailureEnvelop1(self: Self) -> None:
        ...
        # instantiate object
        mohrCoulomb: MohrCoulomb = MohrCoulomb(rockCohesion, frictionAngle)
        normalStressObtained, shearStressObtained = mohrCoulomb.computeFailureEnvelop(
            stressNormalMax, n=10
        )

        # test results
        self.assertSequenceEqual(
            normalStressExpected.tolist(), np.round(normalStressObtained).tolist()
        )
        self.assertSequenceEqual(
            shearStressExpected.tolist(), np.round(shearStressObtained).tolist()
        )
    ...
    def test_computeFailureEnvelop2(self: Self) -> None:
        ...
        mohrCoulomb: MohrCoulomb = MohrCoulomb(rockCohesion, frictionAngle)
        normalStressObtained, shearStressObtained = mohrCoulomb.computeFailureEnvelop(
            stressNormalMax, stressNormalMin, n=10
        )
