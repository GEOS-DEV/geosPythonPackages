#ATS:SequentialMachine machines.summit summitMachine  1
#ATS:summit            machines.summit summitMachine  1
...
class summitMachine( machines.Machine ):
    """
    run using on Summit using jsrun.
    """

    def init( self ):
        self.numtests = 0
        self.numProcsAvailable = 0
        self.logger = logging.getLogger( 'geos-ats' )

    def examineOptions( self, options ):
        "Examine options from command line, possibly override command line choices."
        super( summitMachine, self ).examineOptions( options )
        ...
        self.numberMaxGPUS = ( self.numberMaxProcessors / 42 ) * 6
    ...
    def calculateCommandList( self, test ):
        ...
        ngpu = test.ngpu
        commandList += [ "jsrun", "--np", "%d" % test.np, "-g", "%d" % ngpu, "-c", "1", "-b", "rs" ]
    ...
    def canRun( self, test ):
        ...
        if np * gpusPerTask > self.numberMaxGPUS:
            err = "Too many gpus needed ({np * gpusPerTask:d})"
            self.logger( err )
            return err
