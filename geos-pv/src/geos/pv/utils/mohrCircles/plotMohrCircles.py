...
def _plotMohrCircles(
    ...
    Args:
        ...
        circlesAspect (dict[str, dict[str, Any]]): Dictionnary defining Mohr's circle line properties.
    ...
    for mohrCircle in mohrCircles:
        ...
        xCoords: float = mohrCircle.getCircleCenter() + radius * np.cos( ang )
        yCoords: npt.NDArray[ np.float64 ] = radius * ( np.sin( ang ) )
...
def _plotMohrCoulomb( ax: Axes, mohrCoulomb: MohrCoulomb, curvesAspect: dict[ str, Any ] ) -> None:
    ...
    Args:
        ...
        mohrCoulomb (MohrCoulomb): MohrCoulomb object to define failure envelope parameters.
        curvesAspect (dict[str, Any]): Line properties of the failure envelope.
    """
...
def updateTitle( ax: Axes, userChoices: dict[ str, Any ] ) -> None:
    ...
    Args:
        ...
        userChoices (dict[str, Any]): User-defined properties.
    """
    title: str = userChoices.get( "title", "Mohr's Circles" )
    style: str = userChoices.get( "titleStyle", FontStyleEnum.NORMAL.optionValue )
    weight: str = userChoices.get( "titleWeight", FontWeightEnum.BOLD.optionValue )
    size: int = userChoices.get( "titleSize", 12 )
