...
def SISOFilter(category: FilterCategory, decorated_label: str, decorated_type: str):
    """Decorate single input single output filter."""
    def decorated_class(cls : IsSISOFilter):
        ...
        WrappingClass = smproxy.filter(name=getattr(cls, '__name__', str(cls)), label=decorated_label)(WrappingClass)
