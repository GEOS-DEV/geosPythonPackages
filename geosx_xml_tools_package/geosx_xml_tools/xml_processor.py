...
def merge_xml_nodes(existingNode: ElementTree.Element,
                    targetNode: ElementTree.Element, level: int) -> None:
...
def merge_included_xml_files(root: ElementTree.Element,
                             fname: str,
                             includeCount: int,
                             maxInclude: int = 100) -> None:
    ...
    for includeNode in root.findall('Included'):
        for f in includeNode.findall('File'):
            merge_included_xml_files(
                root, f.get('name'),
                includeCount)  # type: ignore[attr-defined]
    os.chdir(pwd)
